...
async def _recreate_missing_orders(
    pos: dict,
    pos_id: int,
    db_sym: str,
    direction: str,
    ...
    Extrait de sync_all() à l'étape 3 du refactor — logique inchangée, seule la
    liste de paramètres est maintenant explicite (plus de capture implicite de
    variables de la boucle englobante). Prépare le déplacement vers
    execution/protection.py à l'étape 4.
    """
    # ── Recréer les ordres manquants ──
    TOLERANCE = 0.003  # 0.3% pour matching prix (local, était dans sync_all)
...
async def sync_all(config: dict, exchange=None) -> None:
    ...
    try:
        ...
        TOLERANCE = 0.003  # 0.3% pour matching prix
        ...
        for db_sym, pos in db_positions.items():
            ...
            # Stratégie deux-tiers : (1) lookup par order_id en DB → fetch_order
            # (2) fallback matching par stopPrice/type/qty sur open_orders
            # ══════════════════════════════════════════════════════════════
            ...
            await _recreate_missing_orders(
